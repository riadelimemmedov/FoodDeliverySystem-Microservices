...
@ROUTER.post(
    ...
    status_code=status.HTTP_202_ACCEPTED,  # Because our architecture is async
    responses={500: {"model": ConnectError}, 400: {"model": FailedUpdateError}},
)
async def create_order(payload: OrderPayload) -> OrderResponse:
    ...
    async with UnitOfRepositoryWork() as repo:
        service = OrderApiAdapter(
            repo
        )  # Connect to OrderDbAdapter which is connected to MongoDB
