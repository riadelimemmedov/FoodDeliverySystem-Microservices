...
class MongoRepository:
    ...
    # !---------------------------------------------------------
    #
    def __init__(self, session: AsyncIOMotorClientSession):  # type: ignore
    ...
    async def _update(self, payload: OrderModel) -> bool:
        ...
        response = await self.session.client.api_db.orders.update_one(
            {"_id": str(payload.id)}, {"$set": {**base}}
        )
    ...
    async def create(self, payload: OrderModel) -> bool:
        ...
        """
        response = await self.session.client.api_db.orders.insert_one(
            payload.to_mongo()
        )
    ...
    async def read_all(self) -> List[OrderModel]:
        ...
        async for item in self.session.client.api_db.orders.find({}).sort(
            "created", DESCENDING
        ):
